...
Page({
    ...
    data:{
         avatarUrl:'../../assets/images/R-C (1).jpg'
    },

    //获取微信头像
    async chooseavatar(event) {
      //目前获取的微信头像是临时路径
      //临时路径是有失效时间，在实际开发中，需要将临时路径上传到公司服务器
      ...
      //   success:(res) => {
      //     //服务器返回的数据是 JSON 字符串，在使用的时候，需要进行转换 JSON.parse进行转换
    ...
    async handler () {
      ...
      //   /index/findBanner 可以直接访问
      //   /cart/getCartList 需要访问令牌 token 报 208 错误
      //   /index/findBanner111错误地址
  ...
  handler1 (){
    ...
    wx.request({
      ...
      timeout:100,
      success:(res) =>{
        //在使用wx.request 发送请求时，只要成功接受到服务器返回的结果
        //无论 statusCode 状态码是多少，都会执行 success
        //开发者就需要根据业务逻辑，自己来进行相关的判断处理
        console.log(res)
      },
      fail:(err) =>{
        //一般在网络出现异常（网络超时），才会执行 fail
        console.log('网络超时了，这时候会执行fail')
        console.log(err)
      }
    })
  },
  ...
  async allHandler(){
         ...
         //async 和 await 能够控制异步任务已同步的流程来执行

         //  async 和 await 发起多个请求,当地一个请求结束以后，才能够发起第二个请求
        ...
        //演示通过 Promise.all 同时发起多个请求， Promise.all 能够将多个请求同时进行发送
        //Promise.all 能够将多个异步请求同时进行发送，也就是并行发送，并不会造成请求的阻塞，从而不会影响页面的渲染
